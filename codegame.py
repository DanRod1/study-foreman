...
# debug formatage
pp = pprint.PrettyPrinter(stream=sys.stderr, indent=6, width=120)
...
def processRobots(robots={},activeRobots=0,stats={},cibles={},enemies=[],hauteur=0,largeur=0 ) :
    ...
    for key in availableRobots :
        ...
        pp.pprint('NEAR value :')import sys
import re
...
def getBoxByRegion(cibles={}) :
    ...
    count = 0
    taux = {}
    print(f'Regions == {regions} count == {count} ',file=sys.stderr, flush=True)
    for i in regions :
...
def getStatByRobots(robots = {} ) :
    print(f'STATS ROBOTS == { robots }',file=sys.stderr, flush=True)
...
while True:
    ...
    #pprint(f'monMatos : {monMatos} vs sonMatos : {sonMatos} ', file=sys.stderr, flush=True)
    ...
    tour += 1
        ...
        for i in possibleChoice:
            ...
            robots[key]['tour'] = tour + distance
            break
        #pprint(f'POSITION == { position }',file=sys.stderr, flush=True)
        #pprint(f'CHOICE == { choice }',file=sys.stderr, flush=True)
        if 'position'  in locals():
            action += f" {position}"      
        after = len(action)
        #pprint(f'Action == { action }',file=sys.stderr, flush=True)
        if after - before > 0 :
            robots[key]['action'] = action
        else :
            robots[key]['action'] = ''
...
while True:
    ...
    if tour > 0 :
        stats = getStatByRobots(processedRobots)
        pp.pprint('STATISTIQUE UPDATED')
        pp.pprint(stats)
    else :
        stats = {   'rigth' : 0 , 
            'left' : 0 , 
    ...
    pp.pprint("PROCESSED ROBOTS :")
    pp.pprint(processedRobots)
    move = ''
    for key in processedRobots.keys() :
        if len(processedRobots[key]['action']) > 0 :
            move += f"{ processedRobots[key]['action'] };"   
    #pprint(f'Processed Move : {move}',file=sys.stderr, flush=True)
